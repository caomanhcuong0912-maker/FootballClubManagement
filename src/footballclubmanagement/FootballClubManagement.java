...
public class FootballClubManagement {
    static PlayerManager playerMgr  = new PlayerManager();
    static MatchManager  matchMgr   = new MatchManager();
    static SalaryManager salaryMgr  = new SalaryManager(playerMgr, matchMgr); 
    static ReportManager reportMgr = new ReportManager(salaryMgr, matchMgr, playerMgr);
    ...
    List<Player> loaded = fileHandler.loadPlayers();
    for (Player p : loaded) {
        playerMgr.addPlayer(p);
    }

// connect playerManager to trainingManager
trainingMgr.setPlayerManager(playerMgr);
    ...
    while (!back) {
        ...
        try {
            ...
            scan.nextLine();
            switch (choice) {
                case 1 -> System.out.println("[Add Player - coming soon]");
    ...
    while (!back) {
        ...
        try {
            ...
            switch (choice) {
                ...
                case 3 -> matchMgr.createMatchRecord();
                case 4 -> matchMgr.addOrUpdatePerformance();
                case 6 -> matchMgr.viewMatchHistory();
                case 0 -> back = true;
                default -> System.out.println("Invalid option.");
            }
        } catch (InputMismatchException e) {
    ...
    try {
        ...
        switch (choice) {
            case 1 -> {
                ...
                fileHandler.savePlayers(playerMgr.getAllPlayers()); // ← add this
